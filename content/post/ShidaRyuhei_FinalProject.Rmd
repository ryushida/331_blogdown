...
I scraped data from an online database using the rvest package.
...
for(league in 1:length(leagues_teams)) {
  ...
  for(team in 1:length(leagues_teams[[league]])) {
    s = html_session(base_urls[[league]]) %>% jump_to(leagues_teams[[league]][team]) %>% read_html()
  ...
  for(i in 1:length(team_names)) {
    # Add team name
    team_squads[i] <- lapply(team_squads[i], function(x) cbind(x, Team = team_names[[i]]))
    
    # Remove "Players no longer at this club"
    team_squads[i] <- lapply(team_squads[i], head, n= which(as.data.frame(team_squads[i])$Number == "Players no longer at this club")-1)
  #}
...
After I had data frames for each league, I added the league name as a column, and removed numbers that were missing.
...
```{r, message=FALSE}
#all_teams2 <- read_csv("all_teams.csv", locale = readr::locale(encoding = "latin1"))
```

```{r, eval=FALSE}
# Add information for some notable missing players

#all_teams2[all_teams2$Name == "Mohamed Salah", "lat"] = 30.940491
#all_teams2[all_teams2$Name == "Mohamed Salah", "long"] = 30.830231
#all_teams2[all_teams2$Name == "Bukayo Saka", "lat"] = 51.525
#all_teams2[all_teams2$Name == "Bukayo Saka", "long"] = -0.3325
```
...
```{r}
#top4leagues3$`Date of Birth` = dmy(top4leagues3$`Date of Birth`)
```
```{r}
#top4leagues3
```
...
```{r}
#all_teams4$`Date of Birth` = dmy(all_teams4$`Date of Birth`)
```


```{r}
#all_teams4$bd_year <- year(all_teams4$`Date of Birth`)
```
...
#  ggtitle("Birthday of Premier League Players")
```

```{r}
#summary(all_teams4$`Date of Birth`)
```
